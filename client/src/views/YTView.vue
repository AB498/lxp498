...
if (!window.glb.videoInfo) window.glb.videoInfo = {};
...
watch([() => route.params.slug, initialLoad], async () => {
  ...
  if (!player.value) player.value = await getOrMakePlayer();
    ...
    if (!window.YT) {
      ...
      window.onYouTubeIframeAPIReady = () => {
        ...
        let player = new window.YT.Player(playId, {
          ...
          events: {
            onReady: () => playerReadyCallbacks.value.forEach((cb) => cb()),
    ...
    if (newVal == 1) {
      words.value = (
        await window.glb.safeAuthedReq(
          "/api/getYTSubtitles/" + route.params.slug
        )
      ).map((word, i) => {
        word.el = null;
        word.active = false;
      ...
      let translatedWords = await window.glb.safeAuthedReq(
        "/api/getTranslation",
        {
          ...
          targetLang:
            ...
            window.glb.syncerObj.openYTVideo.getMostVotedLanguage,
        }
      );
...
<template>
  <div class="flex items-center justify-center h-full">
    <div class="flex flex-wrap h-full w-full relative">
      ...
      >
        <div
          class="themed-bg-primary shadow-md sm:h-96 h-80 w-full flex flex-col border pointer-none top-0 items-center z-10 shrink-0"
        >
          <!-- video -->
          <iframe
            id="ytPlayerElement"
            class="w-full h-full themed-bg-primary"
            :src="
          ...
          <div class="w-full h-20 subtitles shrink-0">
            <div
              ...
              id="subWordsHolderId"
              v-if="
                window.glb.syncerObj?.openYTVideo?.subtitlesStatus == 1 &&
            ...
            <div
              v-else-if="
                window.glb.syncerObj?.openYTVideo?.subtitlesStatus == 0
              "
            ...
            >
              ...
              <q-slider
                ...
                v-model="
                  window.glb.syncerObj.openYTVideo.subtitlesGenerationProgress
                "
                :min="0"
                :max="100"
              />
            ...
            <div
              v-else-if="
                window.glb.syncerObj?.openYTVideo?.subtitlesStatus == -2
              "
              class="w-full h-full flex items-center justify-center"
            >
              <PopperComponent>
          ...
          >
            ...
            <i
              class="fa center effects w-8 h-8 fa-volume-up"
              @click="toggleMute"
            ></i>
            <div class="grow"></div>
            <div
            ...
            <div
              ...
              @click="
                window.glb.openSelectLang(
                  { multiselect: false, startingPoint: null },
                  (e) => {
                    window.glb.syncerObj.openYTVideo.voteLanguage =
                      e.languagecode;
                  }
                )
              "
            >
              ...
              <div class="flex text-sm">
                {{ window.glb.syncerObj?.openYTVideo?.mostVoted || "unknown" }}
              </div>
            </div>
            <i class="center effects w-8 h-8 fa fa-comment"></i>
            <i
              class="center effects w-8 h-8 fa fa-closed-captioning"
        ...
        <div
          class="info p-2"
          v-if="window.glb.syncerObj.openYTVideo?.videoInfo"
        >
          <div class="text-xl">
            {{ window.glb.syncerObj.openYTVideo?.videoInfo.title }}
          </div>
          <div class="hidden">
            {{ window.glb.syncerObj.openYTVideo?.videoInfo.info.description }}
          </div>
          <div class="">
            {{ window.glb.syncerObj.openYTVideo?.videoInfo.info.channelTitle }}
          </div>
      ...
      >
        ...
        <div class="center flex flex-col full">
          <div
            v-if="window.glb.syncerObj?.openYTVideo.commentsError"
          ...
          >
            <CommentsComponent
              :comments="window.glb.syncerObj?.openYTVideo?.comments"
              v-for="(comment, index) in window.glb.syncerObj?.openYTVideo
                ?.comments?.items"
              :key="index"
              :comment="{
        ...
        <div class="w-full p-2 fixed hidden bottom-0 z-10 themed-bg-secondary">
          <div class="flex-grow">
            <input
              type="text"
