...
async function addProgressCallback(selectedLang) {
  let [err, res] = await window.glb.safeAsync(
    axios.post(
      glb.baseUrl + "/api/createProgress",
      {
    ...
    <div class="w-full h-full flex">
      <div class="flex flex-col h-full basis-1/12">
        <q-tabs
          ...
          dense
          align="left"
          class="themed-bg-secondary shadow-2 w-full"
          :breakpoint="0"
        >
          <div
            v-for="([key, langPrg], index) in Object.entries(langPrgs)"
            :key="langPrg.language"
          >
            <q-tab
              :name="langPrg.language"
              @click="router.push('/progress/' + langPrg.language)"
            >
              <div class="flex flex-col space-x-2 center-cross">
                <img
                  :src="
                    'https://flagcdn.com/256x192/' +
                    Object.values(langImp).find(
                      (l) => l.iso6393 == langPrg.language
                    ).countrycodes[0] +
                    '.png'
                  "
                  class="w-6 h-6"
          ...
          <div class="w-full center p-2">
            <q-icon
              ...
              @click="window.glb.openSelectLang(null, addProgressCallback)"
            ></q-icon>
          </div>
        </q-tabs>
      </div>
      <div class="center full basis-11/12" v-if="route.path == '/progress'">
        <div
          class="flex flex-col p-3 sm:p-16 themed-bg-tertiary rounded-xl center-cross"
        >
