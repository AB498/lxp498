...
async function sendRequest(endpoint) {
    ...
    try {
        console.log(endpoint.params.reduce((acc, cur) => acc.replace(':' + cur.key, (cur.value)), endpoint.url))x
                    ...
                    <div class="flex w-full  items-center space-x-2 p-2 hover-ripple-fast"
                        ...
                        <div class="bg-slate-700 p-1 rounded" v-text="'/test' + endpoint.url"></div>
                            <div v-for="param in endpoint.params" v-if="endpoint.params">
                                {{ param }}
                                                    <q-input v-model="param.value" outlined dense class="bg-slate-700 text-white" />    
                        </div>
                        <div class="grow"></div>
