...
onMounted(async () => {
    apiEndpoints.value = await (await fetch('http://lanxplore.xyz/admin/apis')).json()
...
<template>
        <div class="w-full h-full bg-red-400 overflow-auto">
                <div class="w-full h-full bg-red-400 overflow-auto" v-if="apiEndpoints && apiEndpoints.length > 0">
                        ...
                        <div class="flex flex-col bg-fuchsia-950">
                            <div class="flex w-full  items-center space-x-2 p-2 hover-ripple-fast"
                                ...
                                <div v-for="param in endpoint.params" v-if="endpoint.params">
                                    <q-input v-model="param.value" outlined dense class="bg-slate-700 " :label="param.key"
                                        dark></q-input>
                                </div>
                                <div class="grow"></div>
                            ...
                            <div class="flex flex-col w-full items-stretch  transition-all p-2 "
                                :class="endpoint.show ? 'h-auto' : 'h-0 opacity-0 pointer-events-none overflow-hidden'">
