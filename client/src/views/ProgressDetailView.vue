...
const incPrg = window.glb.user.Progresses.find(
  (l) => l.language == route.params.id
);
import langsImp from "../assets/json/langs_out.json";
let langs = ref(
  Object.entries(langsImp).map(([key, value]) => {
    return {
...
watch([() => route.params.id, initialLoad], async () => {
  const [err, res] = await window.glb.safeAsync(
    axios.post(
      glb.baseUrl + "/api/getProgress",
      {
        language:
          langs.value.find((l) => l.iso6393 == lang.value)?.iso6391 || null,
      },
      {
  ...
  window.glb._nonPersistant.quizView.language = langs.value.find(
    (l) => l.iso6393 == lang.value
  );
...
<template>
  <div
    ...
    v-if="window.glb._nonPersistant.quizView.language"
  >
    <div
      class="sticky z-10 top-0 p-3 themed-bg-secondary shadow text-xl flex center-cross"
    >
      <div
        class="s center p-2 text-xl hover:bg-gray-600 hover-ripple rounded mx-2"
        @click="router.go(-1)"
      >
    ...
    <div
      class="full overflow-auto center-cross flex flex-col sm:flex-row sm:flex-wrap"
    >
      <div
        v-for="(quiz, index) in progresses"
        :key="quiz.id"
      ...
      <div
        v-if="
          window.glb._nonPersistant.quizView.quizzes.find(
            (l) => l.id == route.params.quizId
          )
        "
        class="full"
      >
