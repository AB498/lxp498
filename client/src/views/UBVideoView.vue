...
watch(() => window.glb.syncerObj?.openUBVideo?.subtitlesStatus, async (newVal, oldVal) => {
  if (newVal == 1) {
    words.value = (await window.glb.safeAuthedReq('/api/uploadbase/getUBSubtitles/' + route.params.slug)).map((word, i) => {
      word.el = null;
      word.active = false;
  ...
  <div class="flex items-center justify-center h-full">
    <div class="flex flex-wrap h-full w-full   relative">
      <div class="video-and-suggestions flex flex-col sm:basis-4/6 h-full  overflow-auto">
        ...
        <div class=" sm:h-96 h-80 border-4 w-full flex flex-col pointer-none sticky top-0 items-center z-10 shrink-0">
          <!-- video -->

          <div class="full themed-bg-secondary border-b-2 overflow-hidden">
            <video autoplay muted :src="window.glb.baseUrl + '/api/uploadbase/streamVideo/' + route.params.slug"
              class="full " controls ref="player"></video>
          </div>
          <!-- :class="borderColor" -->
          ...
          <div class="yt-video-player-slider-holder w-full -translate-y-2  relative transition shrink-0 hidden">
            ...
            <div class=" yt-video-player-slider bg-gray-700 h-2 absolute " :style="{ width: (videoProgress || 0) + '%' }">
              <!--mousedown -->
              <div
          ...
          <div class="w-full h-20 subtitles themed-bg-secondary   shrink-0 ">
            <div class="flex items-center justify-center flex-wrap overflow-auto w-full h-full scroll-smooth"
              id="subWordsHolderId"
              v-if="window.glb.syncerObj?.openUBVideo?.subtitlesStatus == 1 && glb.isIterable(words) && words.length > 0">
            ...
            <div v-else-if="window.glb.syncerObj?.openUBVideo?.subtitlesStatus == -1"
              class=" w-full h-full flex items-center justify-center">
              <div class="btn" @click="requestSubGen">Generate Subtitles
                <i class="fa fa-bolt"></i>
              </div>
            </div>
            <div v-else-if="window.glb.syncerObj?.openUBVideo?.subtitlesStatus == 0"
              class=" w-full h-full flex flex-col items-center justify-center">
              <div class=" flex items-center px-2 space-x-2">
                <div>
                  Subtitles Generation in Progress
                </div>
                <div>{{ '(' + parseFloat(window.glb.syncerObj?.openUBVideo?.subtitlesGenerationProgress)?.toFixed(2) +
                  '%)' }}
                </div>
                <LoadingSpin />
              </div>
              <q-slider thumb-size="0px" class="px-6" :color="window.glb.dark ? 'white' : 'black'"
                v-model="window.glb.syncerObj.openUBVideo.subtitlesGenerationProgress" :min="0" :max="100" />

            </div>
            <div v-else-if="window.glb.syncerObj?.openUBVideo?.subtitlesStatus == -2"
              class=" w-full h-full flex items-center justify-center">
              <PopperComponent>
                <template #tohover>
        ...
        <div class="info p-2" v-if="window.glb.syncerObj.openUBVideo.videoInfo">
          ...
          <div class="text-xl">{{ window.glb.syncerObj.openUBVideo.videoInfo.title }}</div>
          <div class="">{{ window.glb.syncerObj.openUBVideo.videoInfo.UserId || 'None' }}</div>
          <div class="hidden">{{ window.glb.syncerObj.openUBVideo.videoInfo.description || 'No description' }}</div>
      ...
      <div
        ...
        <div class="center flex flex-col full">
          ...
          <div v-if="window.glb.syncerObj?.openUBVideo.commentsError" class="full center dark">
            {{ window.glb.syncerObj?.openUBVideo.commentsError }}
          </div>
          <div class="comments flex-col  h-full w-full" v-else-if="window.glb.syncerObj?.openUBVideo?.comments?.items">
            <CommentsComponent :comments="window.glb.syncerObj?.openUBVideo?.comments"
              v-for="(comment, index) in window.glb.syncerObj?.openUBVideo?.comments?.items" :key="index" :comment="{
