...
watch([() => route.params.slug, initialLoad], async () => {
  ...
  let res = await window.glb.safeAuthedReq('/api/uploadbase/addHistory', { id: route.params.slug });
  let videoInfo = await window.glb.safeAuthedReq('/api/uploadbase/getVideoInfo', { id: route.params.slug });
})
    ...
    <div class="flex bg-gray-900 text-white flex-wrap h-full w-full">
      <div class="video-and-suggestions flex flex-col sm:basis-4/6 overflow-auto h-full">
        ...
        <div class=" sm:h-96 h-80 w-full flex flex-col pointer-none sticky top-0 items-center z-10 shrink-0">
          ...
          <video :src="window.glb.baseUrl + '/api/uploadbase/streamVideo/' + route.params.slug" class="w-full shrink-0"
            controls></video>
