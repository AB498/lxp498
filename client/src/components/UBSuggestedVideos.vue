...
onMounted(async () => {
  videos.value = await (
    await fetch(window.glb.baseUrl + "/api/uploadbase/getSuggestedVideos")
  ).json();
  videos.value;
});
...
<template>
  <div class="flex flex-col sm:flex-row sm:flex-wrap items-stretch full">
    <div
      ...
      :key="index"
      class="sm:basis-1/4 sm:overflow-auto w-full p-4"
    >
      <VideoCardVertical
        @click="$router.push('/uploadbase/watch/' + video.id)"
        :video="video"
      />
    </div>
  </div>
