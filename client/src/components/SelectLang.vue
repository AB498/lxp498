...
async function select() {
  ...
  if (!multiselect.value) {
    ...
    for (let i = 0; i < langs.value.length; i++) {
      if (langs.value[i].checked) {
        selected = langsOrg.value.findIndex(
          (e) => e.languagecode == langs.value[i].languagecode
        );
        break;
      }
    }
  ...
  if (window.glb.isIterable(selected)) {
    selected = langsOrg.value
      .filter((l) => {
        if (selected.find((e) => e.languagecode == l.languagecode)) return true;
      })
      .map((e) => ({ ...e }));
...
function clcOut() {
  if (window.glb.settings.showSelectLangModal)
    window.glb.settings.showSelectLangModal = false;
}
watch(
  () => window.glb.settings.showSelectLangModal,
  () => {
    nextTick(() => {
...
window.glb.openSelectLang = (opts, cb) => {
  ...
  langs.value.forEach((element) => {
    let sel = startingPoint?.find(
      (l) => l.languagecode == element.languagecode
    );
    if (sel) element.checked = true;
...
<template>
  ...
  >
    ...
    <div
      class="h-5/6 flex flex-col border themed-bg-primary overflow-auto rounded sm:w-96 w-[95%] relative p-2"
      v-click-outside="clcOut"
    >
      ...
      <q-input
        ...
        @update:model-value="searchLang"
        ref="searchInput"
        class="w-full sticky top-0 themed-bg-secondary backdrop-blur-sm z-10"
      ...
      <div class="flex flex-col space-y-3 overflow-auto p-1">
        <div
          ...
          :ref="(el) => task(el, lang)"
          @click="toggle(index)"
          class="flex p-2 items-center rounded relative cursor-pointer"
          v-ripple
          :class="(lang.checked && 'bg-blue-700') || 'themed-bg-secondary'"
        >
          <img
            class="rounded-full w-6 h-6 mx-3"
          ...
          <div class="flex flex-col items-start justify-center overflow-hidden">
            ...
            <div class="flex overflow-auto space-x-1">
              <div
                v-for="countrycode in lang.countrycodes.slice(
                  1,
                  lang.countrycodes.length
                )"
                class="w-4 h-4"
              >
                <img
      ...
      <div class="grow"></div>
      <div
        class="sticky bottom-0 w-full p-2 themed-bg-tertiary flex items-stretch justify-center"
      >
        <div
          v-ripple
          class="hover-ripple basis-full flex items-center justify-center cursor-pointer"
          @click="clcOut"
        >
          Cancel
        </div>
        <div
          v-ripple
          class="hover-ripple basis-full flex items-center justify-center btn"
          @click="select"
        >
          Select
        </div>
