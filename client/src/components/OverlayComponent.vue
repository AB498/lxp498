...
<template>
  <div
    :class="
      (mainStore.showLangModal && 'opacity-100 pointer-events-auto') ||
  ...
  >
    ...
    <div
      class="h-5/6 bg-slate-900 overflow-auto rounded sm:w-80"
      v-click-outside="clcOut"
    >
      ...
      >
        ...
        <img
          ...
          :src="'https://flagcdn.com/h60/' + lang.code + '.png'"
        />
        <div>{{ lang.languagename }} ({{ lang.code.toUpperCase() }})</div>
      </div>
      <div
        class="w-full sticky bottom-0 p-2 bg-red-900 flex items-stretch justify-center"
      >
        <div
          class="basis-full flex items-center justify-center"
          @click="clcOut"
        >
          Cancel
        </div>
        <div
          class="basis-full flex items-center justify-center btn"
          @click="vote"
        >
          Vote
        </div>
