...
<template>
    <div class=" flex flex-col  w-full relative hover:-translate-y-1 shadow-md transition h-full rounded-xl break-words">
        ...
        <div class="w-full p-4 break-words overflow-auto">
            <div class="w-full text-ellipsis overflow-auto text-xs  break-words" v-text="video.title || 'No Title'"></div>
            <div class="w-full text-ellipsis overflow-auto text-xs themed-text-secondary  break-words" v-text="video.description || 'No Description'">
            </div>
        </div>
    </div>
</template>