...
<template>
    <div class=" flex flex-col  w-full relative hover:-translate-y-1 shadow-md transition h-full rounded-xl">
        ...
        <div class="w-full p-4 break-words">
            <div class="w-full text-ellipsis overflow-ellipsis text-xs line-clamp-3 break-words" v-text="video.title || 'No Title'"></div>
            <div class="w-full text-ellipsis overflow-ellipsis text-xs themed-text-secondary line-clamp-2 break-words" v-text="video.description || 'No Description'">
            </div>
        </div>
    </div>
</template>