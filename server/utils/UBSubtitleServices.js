...
const s = require("../s");
const downloadedVideosDirectory = join(rootDirectory, "ubuploadedvideos");
...
class SubtitleServices {
  ...
  async addCallback(videoId, callback, email) {
    let subGenProg =
      (await models.UBVideo.findOne({ where: { id: videoId } })).get({
        plain: true,
  ...
  async generateSubtitles(videoId, lang, targetLang = null) {
    ...
    let foundvideo = await models.UBVideo.findOne({ where: { id: videoId } });
    if (!foundvideo) {
    ...
    console.log("video: " + foundvideo.fileName);
    let downloadedFile = foundvideo.fileName;
...
if (!global.__ubVideoAPIServices) {
  global.__ubVideoAPIServices = new SubtitleServices();
}
let ubVideoAPIServices = global.__ubVideoAPIServices;

module.exports.ubVideoAPIServices = ubVideoAPIServices;
